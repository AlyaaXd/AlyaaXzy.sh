...
fake = '𝑪𝒓𝒆𝒂𝒕𝒐𝒓 𝑩𝒐𝒕 𝑨𝒍𝒚𝒂𝒂𝑿𝒛𝒚 𝑿 𝐵𝑂𝑇𝐶𝐴𝐻™ '
fake1 = '𝑨𝒍𝒚𝒂𝒂𝑿𝒛𝒚 𝑿 𝐵𝑂𝑇𝐶𝐴𝐻'
numbernye = '0'
waktu = '-'
alasan = '-'
...
module.exports = xdev = async (xdev, dev) => {
	try {
        ...
        const fakethumb = (teks, yes) => {
            xdev.sendMessage(from, teks, image, {thumbnail:fs.readFileSync('./stik/AlyaaXzy.jpeg'),quoted:dev,caption:yes})
        }
...
await xdev.sendMessage(from, gbuttonan, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./stik/AlyaaXzy.jpeg'),
...
break
    ...
    if (!dev.key.fromMe && !isOwner) return reply(mess.owner)
			...
			delb = await xdev.downloadMediaMessage(boij)
			fs.writeFileSync(`./stik/AlyaaXzy.jpeg`, delb)
			fakestatus('Sukses')
        	} else {
...
xdev.sendMessage(from,'Nih Kak Creator AlyaaXzy',text,{quoted: hehe})
}
if (button == 'GITHUB') {
console.log('GITHUB')
xdev.sendMessage(from, `*Nih kak github AlyaaXzy*\n*Scnya free kak dan juga free api 🤗*\n*Ingat ! jangan diperjual belikan yah !*\n\nhttps://github.com/AlyaaXd`, text, {quoted: dev})
}
if (button == 'REST API') {
